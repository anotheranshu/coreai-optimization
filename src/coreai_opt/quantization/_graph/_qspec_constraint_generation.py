...
Every constraint the reconciler applies is emitted here. The pipeline
consults this module twice:

    * Once during seeding — every fx node in the graph is passed to
      :func:`_generate_constraints_for_node` to build the initial
      queue.
    * Then on every re-enqueue — after a constraint mutates the state
      of a slot, every fx node touching that slot's node is passed
      back through :func:`_generate_constraints_for_node` so that a
      newly-populated slot can trigger sharing constraints its neighbors
      had previously suppressed.

The three constraint sources dispatched from here are:

    * Adjacent-edge sharing (producer.OUTPUT ↔ consumer.INPUT[i]) —
      :func:`_adjacent_edge_constraints`.
    * Shared-observer patterns (concat, flatten, pooling) —
      delegated to the pattern class via
      :meth:`SharedObserverModulePattern.generate_qspec_sharing_constraints`.
    * Shared-state (shared-weight) consumers —
      :func:`_shared_state_constraints`.

The context bundle :class:`_AnnotationContext` that every generator
takes is also defined here — it's the "inputs to constraint generation"
type — and :func:`_nodes_covered_by` lives here because the quantizer
uses it to enumerate a match's covered nodes when building that context.
"""
...
from ._annotation_pattern_registry import SharedObserverModulePattern
from ._annotation_utils import _get_call_function_node_from_partition
from ._qspec_constraints import Constraint, ShareObserverInstance
from ._qspec_types import NodeSlot, ProvisionalQSpecMap, SlotKind


# ---------------------------------------------------------------------------
# Context bundle (Phase-1/2 outputs, consumed by every generator).
# ---------------------------------------------------------------------------
...
class _AnnotationContext:
    ...
    Attributes:
        ...
        shared_observer_nodes (dict[fx.Node, type[SharedObserverModulePattern]]):
            ...
            :func:`_generate_constraints_for_node` dispatches through
            this mapping.
    """
